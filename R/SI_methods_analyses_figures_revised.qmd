...
subtitle: "Supplementary Material - data and code"
date:  "2024-03-29"
...
library(galah) #for gathering record counts from Living Atlases
```
...
length(unique(dat$DOI)) #2171 unique values
...
```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 10 # Height for figures in inches
df %>% 
...
Plot frequencies of manual tags for ALA use type.   
...
Authors that contributed to  both articles and grey literature as % of the total number of unique author names: `r round(length(intersect(unique(authors_articles), unique(authors_grey))) / length(unique(authors_all)) *100, 1)`       

Models for historical author team size changes for articles and grey literature.    

```{r}
# ## Author numbers per paper across years
# #create a data frame of author counts and literature type
dat %>%
  group_by(article_grey) %>%
...
```{r}
#| eval: FALSE
load(here("data", "data_a1.RData")) #load saved data object
...
metrics_article <- data.frame(type = rep("article", length(igraph::degree(a1is))),
  ...
  eig = igraph::eigen_centrality(a1is)$vector,
  cor = igraph::coreness(a1is)
  )
...
```{r}
#| eval: FALSE
#load(here("data", "data_g1.RData")) #load saved data object
...
metrics_grey <- data.frame(type = rep("grey", length(igraph::degree(g1is))),
  ...
  eig = igraph::eigen_centrality(g1is)$vector,
  cor = igraph::coreness(g1is)
  )
...
Summary of results from network analyses:
We note that author team size potentially influences the ‘connectedness’ of the co-authorship network among the ALA-related outputs analysed above. The plotted network graph has nodes (vertices) representing outputs of two types (article, grey) which are connected via the author identity (edges). Formal graph analysis indicated that the co-authorship network of grey outputs had a relatively higher proportion of weakly connected items than an equivalent network of journal articles. A community detection algorithm revealed lower modularity for a grey outputs graph in comparison to journal articles (0.40 vs. 0.76) indicating that clustered journal articles are more tightly connected to each other by co-authorship, potentially via multiple shared authors.
...
(Can be run from a saved data object).  
...
#How many papers with only Australian authors?
#################View(doi_country_list[lengths(lapply(doi_country_list, unique)) == 1])
doi_country_list2 <- sapply(doi_country_list, unique) #unique country names
table(unlist(doi_country_list2[lapply(doi_country_list2, length) == 1]) == "AUSTRALIA") # Filter out elements with length 0, unlist and then check which are "AUSTRALIA" = 1030/1765 #58% of articles with only Australian authorship 
...
small_matrix <- net_matrix[rownames(net_matrix) %in% top10countries$doi_country_all, colnames(net_matrix) %in% top10countries$doi_country_all] #reduce matrix to most productive countries (with highest total number of authors)
...
Plot chord diagram to pdf.   

```{r}
#| eval: FALSE
pdf(file = here("plots", "Fig4_rev_plot.chord_country_coauthorship.pdf"), width=8, height=8, pointsize=12)
par(mar=c(0,0,0,0))
par(oma=c(0,0,0,0))
circos.clear()
circos.par(canvas.xlim = c(-1, 1), canvas.ylim = c(-1.2, 1.5)) #set margins
...
Plot on the world map and save to pdf. 
...
The articles were published in `r length(unique(dat_articles$Journal_name))` different journals.   
...
#| fig-height: 4 # Height for figures in inches
dat_articles %>% 
  count(Journal_name) %>% filter(n >= 25) -> count_Journal_name #getjournal names with at least 25 articles
...
Plot number of distinct publication journals per publication year.        

```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 4 # Height for figures in inches

#plot number of distinct journal names per year
dat_articles %>% 
  group_by(Publication.Year) %>%
  summarise(n = n_distinct(Journal_name)) %>% ggplot(
       aes(x = str_to_title(Publication.Year), 
           y = n)) +
  geom_point() +
  xlab("Publication year") +
  ylab("Number of distinct journal names per year")
```


Plot trends of number of articles in most represented journals per publication year.        

```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 6 # Height for figures in inches
dat_articles %>% 
  count(Journal_name, Publication.Year) %>% 
  filter(Journal_name %in% 
           count_Journal_name$Journal_name) -> count_Journal_name_Year #get counts per journal and year only for most common journals (n>25)

count_Journal_name_Year %>% ggplot(
       aes(x = str_to_title(Publication.Year), 
           y = str_to_title(Journal_name),
           size = n)) +
  geom_point() +
  xlab("Publication year") +
  ylab("Journals with over 20 articles")
```
...
Plot journals with Impact Factor >15

```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 4 # Height for figures in inches
dat_articles_IF <- readr::read_csv(file = here("data","dat_articles_IF.csv")) #load saved fetched data
...
Plot journal Impact Factor values per publication year.    
...
dat_articles_IF %>% 
  ...
  #summarise(n = n_distinct(Journal_name)) %>% 
  ggplot(
       aes(x = Publication.Year, 
           y = Journal_IF)) +
  geom_jitter(position = position_jitter(width = 0.3, height = 0.1), alpha = 0.5) +
  xlab("Publication year") +
  ylab("Journal Impact Factor")
```


Plot journal Impact Factor values versus article counts.    

```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 6 # Height for figures in inches

#plot article count versus journal impact factor
dat_articles_IF %>% 
  filter(!is.na(Journal_IF)) %>% 
  count(Journal_IF) %>% 
  ggplot(
...
Summarise Best Quartile data from Scimago.  

```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 2 # Height for figures in inches
...
Summarise Journal Region data from Scimago.   
...
```{r}
OA_df <- readr::read_csv(file = here("data","OA_article_data_Unpaywall.csv")) #load saved data object
# names(OA_df)
...
```{r}
dat_articles_CR <- readr::read_csv(file = here("data","dat_articles_CR.csv"))  #load saved merged data frame
...
```{r}
#| fig-width: 8 # Width for figures in inches
#| fig-height: 5 # Height for figures in inches
...
```{r}
Altmetrics.results_df <- readr::read_csv(file = here("data","Altmetrics.results_df.csv")) #load saved data file
...
#table(is.na(Altmetrics.results_df$score)) #no NA - overall composite altmetric score
...
##get an overview of the Living Atlases 
atlas_counts()
...
#galah_config(atlas = "ALA")  #just for Australia

## print out cutrrent record counts for each Living Atlas, by region
for(i in 1:length(atlases$region)) {     
  galah_config(atlas = atlases$region[i])
  out <- paste0(atlases$region[i], " = ", atlas_counts())  # Some output
...
#### Figure 4 - global country collaboration.  
...
#### Figure 5 - journal impact factor, popularity, regionality.   
...
#plot article count versus journal impact factor
figure5A <- dat_articles_IF %>% 
  filter(!is.na(Journal_IF)) %>% 
  count(Journal_IF) %>% 
  ggplot(
       aes(x = (Journal_IF), 
           y = n)) +
  geom_jitter(position = position_jitter(width = 0.3, height = 0.1), alpha = 0.5, color = "#66c2a5") +
  xlab("Journal Impact Factor") +
  ylab("Article count") 
```
...
figure5B <- ggplot(count_Journal_name, aes(x = reorder(Journal_name, n), y = n)) + 
  geom_bar(stat = "identity", fill = "#66c2a5") +
  coord_flip() +
  theme(plot.margin = margin(0, 0, 0, 0))+
  xlab("Most represented journals")  +
  ylab("Article count")
...
#using package ggmosaic
figure5C <- dat_articles_IF_SJR %>% filter(!is.na(SJR_Best_Quartile)) %>% ggplot() +
  geom_mosaic(aes(x = product(Regions, SJR_Best_Quartile), fill = Regions)) +
  theme_mosaic() + 
...
#save the figure 
ggsave(plot = figure5, here("plots", "plot.journal_characteristics_rev.pdf"), width = 16, height = 18, units = "cm", dpi = 300, scale = 1.6)
```


#### Figure 6 - open access status.
...
dim(dat_articles_OA)
names(dat_articles_OA)
table(is.na(dat_articles_OA$is_oa)) #no information for 595 articles
...
figure6A <- 
  count_dat_articles_OA %>%
  arrange(Count) %>%    # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(is_oa2 = factor(is_oa, levels = c("no information", "closed", "open access"))) %>%   # This trick update the factor levels
  ggplot(aes(x = is_oa2, y = Count, fill = oa_status)) + 
  geom_bar(stat = "identity") +
  coord_flip() +
  xlab("")  +
...
figure6B <- ggplot(count_year_oa, aes(x = as.character(Publication.Year), y = n, fill = reorder(is_oa, n))) + 
...
figure6 <- figure6A / figure6B +
  ...
  plot_annotation(tag_levels = "A")
#save the figure 
ggsave(plot = figure6, here("plots", "plot.oa_status.pdf"), width = 16, height = 12, units = "cm", dpi = 300, scale = 1.6)
```

New Figure 2 (added after manuscript revision).    
Note this figure is generated using internal files from The ALA and data gathered from GBIF via API.
